...
namespace Nop.Plugin.Payments.PayPalStandard.Controllers
{
    ...
    public class PaymentKNETStandardController : BasePaymentController
    {
        #region Fields
        ...
        public PaymentKNETStandardController(IGenericAttributeService genericAttributeService,
        ...
        public async Task<IActionResult> Configure()
        {
            ...
            var model = new ConfigurationModel
            {
                ...
                TranportalId= payPalStandardPaymentSettings.TranportalId,
                TranportalPwd=payPalStandardPaymentSettings.TranportalPwd,
                TerminalResourceKey=payPalStandardPaymentSettings.TerminalResourceKey
    };
        ...
        public async Task<IActionResult> Configure(ConfigurationModel model)
        {
            ...
            payPalStandardPaymentSettings.TranportalId = model.TranportalId;
            payPalStandardPaymentSettings.TranportalPwd = model.TranportalPwd;
            payPalStandardPaymentSettings.TerminalResourceKey = model.TerminalResourceKey;
            ...
            await _settingService.SaveSettingOverridablePerStoreAsync(payPalStandardPaymentSettings, x => x.TranportalId, model.TranportalId_OverrideForStore, storeScope, false);
            await _settingService.SaveSettingOverridablePerStoreAsync(payPalStandardPaymentSettings, x => x.TranportalPwd, model.TranportalPwd_OverrideForStore, storeScope, false);
            await _settingService.SaveSettingOverridablePerStoreAsync(payPalStandardPaymentSettings, x => x.TerminalResourceKey, model.TerminalResourceKey_OverrideForStore, storeScope, false);
        ...
        public async Task<IActionResult> PDTHandler([FromForm] string trandata,
...
[FromForm] string result)
        {
            ...
            //if (await _paymentPluginManager.LoadPluginBySystemNameAsync("Payments.KNETStandard") is not PayPalStandardPaymentProcessor processor || !_paymentPluginManager.IsPluginActive(processor))
            ...
            var knet = new KnetUtil(_payPalStandardPaymentSettings);

            var decryptedParams = knet.DecryptUrlParams(trandata);
            var paramsArray = decryptedParams.Split("&");
            Dictionary<string, string> KnetParamsList = new();


            var orderNumber = trackid;
            ...
            var past = "";
            KnetParamsList.TryGetValue("result",out past);
            if (past == "CAPTURED")
            {
                ...
                try
                {
                    mcGross = decimal.Parse(KnetParamsList["amt"], new CultureInfo("en-US"));
                }
                catch (Exception exc)
                {
                ...
                {
                    var errorStr = $"Knet PDT. Returned order total {mcGross} doesn't equal order total {order.OrderTotal}. Order# {order.Id}.";
                    //log
            ...
            else
            {
                ...
                //if (past == "CANCELED")
                //{
                //    return RedirectToRoute("/Plugins/PaymentPayPalStandard/CancelOrder", new { orderId = order.Id });
                //}
